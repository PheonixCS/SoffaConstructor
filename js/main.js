$(document).ready(function(){
    ...
    $updateObjSize = function($idUpdObj){
        ...
        if($('#'+$idUpdObj).hasClass('appendedC')){
            $cOriginWidth = 75;
            $cOriginHeight = 110;
            $cWidth = $updateSize()*$basescale*2.14;
    ...
    $funcCalculate = function($id){ 
        if($('#'+$id).hasClass('mainGr')){
            $top = $('#'+$id).offset().top;
            $left = $('#'+$id).offset().left;
            $right = $left+$('#'+$id).width();
            $bottom =$top+  $('#'+$id).height();
            /// определяем коэфициент различия с реальными размерами
            if($('#'+$id).hasClass('appendedB')){
                $scaleH = $('#'+$id).height()/$realBHeight;
                $scaleW = $('#'+$id).width()/$realBWidth;
            }
            if($('#'+$id).hasClass('appendedBM')){
                $scaleH = $('#'+$id).height()/$realBMHeight;
                $scaleW = $('#'+$id).width()/$realBMWidth;
            }
            if($('#'+$id).hasClass('appendedC')){
                $scaleH = $('#'+$id).height()/$realCHeight;
                $scaleW = $('#'+$id).width()/$realCWidth;
            }
            if($('#'+$id).hasClass('appendedCM')){
                $scaleH = $('#'+$id).height()/$realCMHeight;
                $scaleW = $('#'+$id).width()/$realCWidth;
            }
            if($('#'+$id).hasClass('appendedCB')){
                $scaleH = $('#'+$id).height()/$realCBHeight;
                $scaleW = $('#'+$id).width()/$realCWidth;
            }
            // определеяем фактические параметры
            if($top < $minTop){
                $minTop = $top;
            }
            if($left < $minLeft){
            ...
            //console.log($maxRight,$minTop);
            $realwidth = ($maxRight-$minLeft);
            $realHeight = ($maxBottom-$minTop);
            console.log($scaleH,$scaleW);
            $('#realDepth').text((($realHeight*1/$scaleH/35).toFixed(3)*35).toFixed());
            $('#realWidth').text((($realwidth*1/$scaleW/35).toFixed(3)*35).toFixed());
            $countCost();
        }
    };
    ...
    $creatModul = function(id,$number){
        if ($number == 1){
            modul = '<svg id = "'+id+'" class = "appended-modul appendedB" width="35" height="111" viewBox="0 0 35 111" fill="none" xmlns="http://www.w3.org/2000/svg">'+
        ...
        if ($number == 3){
            modul = '<svg id = "'+id+'" class = "appended-modul appendedC" width="75" height="110" viewBox="0 0 75 110" fill="none" xmlns="http://www.w3.org/2000/svg">'+
            '<mask id="mask0_78_208" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="75" height="110">'+
            '<path d="M0 103.176L0 6.87502C0 5.06619 0.733333 3.33015 2.03333 2.04905C3.33333 0.767947 5.1037 0.0509707 6.94444 0.0509707H67.9704C69.8111 0.0509707 71.5778 0.771586 72.8815 2.04905C74.1852 3.33015 74.9148 5.06619 74.9148 6.87502V103.176C74.9148 104.985 74.1815 106.721 72.8815 108.002C71.5778 109.283 69.8111 110 67.9704 110H6.94444C5.1037 110 3.33704 109.279 2.03333 108.002C0.729629 106.721 0 104.985 0 103.176Z" fill="white"/>'+
            '</mask>'+
            '<g mask="url(#mask0_78_208)">'+
            '<path d="M0 0.0327682L0 110H74.9407V0.0327682H0Z" fill="#DAD9D9"/>'+
            '</g>'+
            '<mask id="mask1_78_208" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="75" height="110">'+
            '<path d="M0 103.172L0 6.82405C0 5.01159 0.733333 3.27554 2.03704 1.99808C3.34074 0.716982 5.10741 5.22208e-06 6.95185 5.22208e-06H68.0481C69.8926 5.22208e-06 71.6593 0.720621 72.963 1.99808C74.2667 3.27918 75 5.01522 75 6.82405V103.172C75 104.985 74.2667 106.721 72.963 107.998C71.6593 109.279 69.8926 109.996 68.0481 109.996H6.95185C5.10741 109.996 3.34074 109.276 2.03704 107.998C0.733333 106.717 0 104.981 0 103.172Z" fill="white"/>'+
